const xml2js = require('util').promisify(require('xml2js').Parser({ trim: true, normalize: true, normalizeTags: true, explicitArray: false }).parseString)
...
const verifyIndexExistsInObjectPerType = (objectKeys, strType) =>
  objectKeys.indexOf(strType) !== -1 && objectKeys[objectKeys.indexOf(strType)] === strType
...
const valueFormatter = arrValue => {
  ...
  if (verifyIndexExistsInObjectPerType(keys, 'boolean')) { return compose(parseInt, normalizeResponsesIntegrator, String)(arrValue[keys[keys.indexOf('boolean')]]) }
  if (verifyIndexExistsInObjectPerType(keys, 'int')) { return compose(parseInt, normalizeResponsesIntegrator, String)(arrValue[keys[keys.indexOf('int')]]) }
